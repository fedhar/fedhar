...
staff_num = database.Realworld.STAFF_3
samples = database.Realworld.SAMPLES_3
...
def drift_train(epochs, user, verbose, show_idx, near_semi=0.2, lr=None, momentum=0.9):
    ...
    drift.RealworldDriftPT(
        ...
        model_path=Path.feder.format(dataset=dataset, staff_num=staff_num,
                                     samples=samples, model='stochastic_near', name='2'),
...
if __name__ == '__main__':
    ...
    for user in range(database.Realworld.CLIENTS - database.Realworld.STAFF_3):
        for near_semi in search_args['near_semi']:
            for lr, momentum in search_args['lr_momentum']:
                drift_train(epoch, user, verbose=False, show_idx=show_idx, near_semi=near_semi, lr=lr, momentum=momentum)